...
test.serial('Return an empty array when there are no aliases in the DB', async (t) => {
  const res = await request(server).get('/api/aliases/alias').expect(200).expect('Content-Type', /json/)
...
test.serial('Get all aliases for an existing tag', async (t) => {
  await AliasSet.create(aliases)
  const res = await request(server).get('/api/aliases/alias/coding').expect(200).expect('Content-Type', /json/)
...
test.serial('Getting aliases for a non-existing tag returns 404', async (t) => {
  await AliasSet.create(aliases)
  const res = await request(server).get('/api/aliases/alias/nonexistingTag')
...
test.serial('Deleting a tag', async t => {
  ...
  await request(server).get('/api/aliases/alias/programming').expect(200).expect('Content-Type', /json/)
  ...
  await request(server).get('/api/aliases/alias/programming').expect(404)

  // The deleted tag is removed from aliases of other tags
  const res2 = await request(server).get('/api/aliases/alias/coding')
  t.false(res2.body.aliases.includes('programming'))
  const res3 = await request(server).get('/api/aliases/alias/development')
...
test.serial('Deleting a tag, non-admin request', async (t) => {
  ...
  const res1 = await request(server).get('/api/aliases/alias/coding')
  t.true(res1.body.aliases.includes('programming'))
  const res2 = await request(server).get('/api/aliases/alias/development')
...
test.serial('Deleting a tag alias', async (t) => {
  ...
  const res1 = await request(server).get('/api/aliases/alias/programming')
  ...
  const res2 = await request(server).get('/api/aliases/alias/programming')
  t.false(res2.body.aliases.includes('coding'))

  // Test bi-directional relationship: TagA is no longer in the alias set of TagB
  await request(server).get('/api/aliases/alias/coding').expect(200).expect('Content-Type', /json/)
  const res3 = await request(server).get('/api/aliases/alias/coding')
  t.false(res3.body.aliases.includes('programming'))
})
...
test.serial('Editting a tag', async (t) => {
  ...
  await request(server).get('/api/aliases/alias/edittedprogramming').expect(200).expect('Content-Type', /json/)
...
test.serial('Editting a tag, non-admin request', async (t) => {
  ...
  await request(server).get('/api/aliases/alias/edittedprogramming').expect(404)
...
test.serial('Adding a tag', async (t) => {
  ...
  await request(server).get('/api/aliases/alias/newtag').expect(404)
  ...
  const res2 = await request(server).get('/api/aliases/alias/newtag').expect(200).expect('Content-Type', /json/)
...
test.serial('Adding a tag, non-admin request', async (t) => {
  ...
  const res = await request(server).get('/api/aliases/alias/newtag').expect(404)
  t.is(res.status, 404)
})
...
test.serial('Adding an existing tag to the alias list of another tag', async (t) => {
  ...
  await request(server).get('/api/aliases/alias/newtag').expect(200).expect('Content-Type', /json/)
  ...
  const res3 = await request(server).get('/api/aliases/alias/newtag')
  t.true(res3.body.aliases.includes('programming'))
})
...
test.serial('Adding a new tag to the alias list of another tag', async (t) => {
  ...
  // TagB is now an alias of tagA
  const res3 = await request(server).get('/api/aliases/alias/programming')
  t.true(res3.body.aliases.includes('newtag'))

  // TagB has been added to the alias collection, and tagA is an alias of tagB
  await request(server).get('/api/aliases/alias/newtag').expect(200).expect('Content-Type', /json/)
  const res4 = await request(server).get('/api/aliases/alias/newtag')
