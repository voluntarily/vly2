...
const callApi = async (endpoint, method = 'get', body, cookies) => {
  ...
  console.log('Call API function got called ')
  console.log('The cookies idToken is not null? ', cookies.idToken != null)
  if (cookies) {
    headers.Cookie = convertCookiesToKvps(cookies)
  }
  try {
    /**
     * Small changes but will solve the log in issues
