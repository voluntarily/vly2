...
export const createPersonFromUser = async (user, cookies) => {
  ...
  try {
    console.log('createPersonFromUser', person)
...
export const getPersonFromUser = async (user, cookies) => {
  ...
  try {
    ...
    console.log(`people/?q=${JSON.stringify(query)}`)
    let person = await callApi(`people/?q=${JSON.stringify(query)}`, 'get', null, cookies)
    if (!person) {
      person = await createPersonFromUser(user, cookies)
    }
    return person
  } catch (err) {
    console.error('getPersonFromUser', err)
  }
}
