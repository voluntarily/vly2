...
test.serial('render registerRequest with registered teacher', async t => {
  ...
  t.is(wrapper.find('span').at(3).text(), 'Teacher Registration Record')
...
test.serial('render registerRequest with un-registered teacher', async t => {
  ...
  t.is(wrapper.find('span').at(3).text(), 'You can validate as a teacher using your teacher registration number:')
})
